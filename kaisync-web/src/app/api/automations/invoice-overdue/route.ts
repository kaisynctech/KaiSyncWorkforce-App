...
// ── POST — trigger for the caller's company (manual or from UI) ───────────────
export async function POST() {
  ...
  const { data, error } = await (supabase.rpc as unknown as RpcFn)(
    'process_overdue_invoices', { p_company_id: emp.company_id }
  )
...
export async function GET(request: Request) {
  ...
  for (const co of companies ?? []) {
    const { data } = await (admin.rpc as unknown as RpcFn)(
      'process_overdue_invoices', { p_company_id: co.id }
    )
    results[co.id] = data
  }
