...
--
-- Current employees_insert and companies_insert policies
-- check only auth.uid() IS NOT NULL — any authenticated user
-- can INSERT any column value including access_level = 'owner'
-- and owner_user_id = <arbitrary uuid>.
--
-- Fix: tighten WITH CHECK on both policies.
-- SECURITY DEFINER RPCs are not bound by RLS INSERT policies
-- and are unaffected.
-- ============================================================
...
-- Effect:
--   Any direct PostgREST POST to /employees with access_level != 'employee'
--   returns a policy violation. HR promotion to hr/manager/owner
--   goes through the ARCH-001 role-management RPC (SECURITY DEFINER).
--   The company_id = ANY(user_company_ids()) check is also new —
--   the original policy had no company scoping at all.

-- ----------------------------------------------------------------
-- companies_insert — prevent nominating a different owner on INSERT
-- (No create_company RPC exists; direct INSERT is the registration
-- path. Case B per the brief applies.)
-- ----------------------------------------------------------------
...
-- Effect:
--   A user can only INSERT a company row where they are the owner.
--   Attempting to register a company with owner_user_id set to
--   another user's UUID returns a policy violation.
