-- Contractor member scope controls:
-- - contractor-level toggle for member visibility
-- - role/primary-aware job scope checks for employee RPCs

ALTER TABLE IF EXISTS public.contractors
  ADD COLUMN IF NOT EXISTS allow_members_view_all_jobs boolean NOT NULL DEFAULT true;
CREATE OR REPLACE FUNCTION public.employee_has_contractor_job_scope(
  p_company_id bigint,
...
AS $$
  SELECT EXISTS (
    ...
    JOIN public.contractors c
      ON c.id = cm.contractor_id
     AND c.company_id = cm.company_id
    WHERE cm.company_id = p_company_id
      AND cm.employee_id = p_employee_id
      AND cm.contractor_id = p_contractor_id
      AND (
        c.allow_members_view_all_jobs = true
        OR cm.is_primary = true
        OR lower(coalesce(cm.role_label, '')) IN ('owner', 'manager', 'lead')
      )
  );
$$;
  ...
  WHERE j.company_id = p_company_id
    AND (
      ...
      OR (
        ...
        AND public.employee_has_contractor_job_scope(
          p_company_id,
          p_employee_id,
          j.contractor_id
        )
