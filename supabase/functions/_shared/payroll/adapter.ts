/**
 * Server-side port of `kaisync-web/src/lib/payroll-engine.ts` — thin adapter over
 * `calculator.ts` that maps the DB's flat `employees` row + `PayrollSettings` prefs
 * shape to the calculator's structured inputs, and back to the `payment_approvals`
 * row shape. Kept logic-identical to the web adapter (relative imports only);
 * `policy_snapshot.source` is `'kaisync-web-payroll-engine-v2-server'` so persisted
 * payslips are distinguishable as server-calculated (non-repudiation).
 */

import type { PayrollSettings } from './prefs.ts'
...
import type { YtdTotals } from './irp5.ts'
import type { SalaryHistoryEntry } from './salary-resolver.ts'
import { buildTemplateAwareSessions } from './sessions-from-punches.ts'
import type { ShiftTemplateLike } from './punch-session.ts'

export type { LeaveSnapshot, PunchLike, SalaryHistoryEntry, ShiftTemplateLike }
...
export function calculatePayslip(input: {
  ...
  const useLeaveOverride =
    leaveRecords.length === 0 && ((input.paidLeaveDays ?? 0) > 0 || (input.unpaidLeaveDays ?? 0) > 0)
  ...
  return {
    ...
    policy_snapshot: {
      source: 'kaisync-web-payroll-engine-v2-server',
    ...
    audit_log: [
      {
        action: 'calculated',
        detail: `Server engine (${payBasisOut}); gross ${result.grossPay}; net ${result.netPay}`,
